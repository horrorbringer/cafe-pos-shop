<div class="flex h-screen bg-stone-50 text-stone-800 select-none" x-data="{ addingFlash: null }" x-on:item-added.window="addingFlash = $event.detail.productId; setTimeout(() => addingFlash = null, 400)">
        ...
        <div class="bg-white border-b border-stone-200 px-5 py-3 space-y-3 shrink-0">
            ...
            <div class="flex items-center gap-3">
                ...
                @if($orderType === 'dine_in')
                    <div class="relative">
                        <input type="text" wire:model.live="tableNumber" placeholder="Table #"
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-6">
            ...
            @if($popularProducts->isNotEmpty() && $selectedCategoryId === 0 && empty($search))
                <div>
                    <div class="flex items-center gap-1.5 mb-3">
                        ...
                        <h3 class="text-xs font-semibold text-stone-500 uppercase tracking-wider">Popular Items</h3>
                    </div>
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
                        @foreach($popularProducts as $product)
                            ...
                            @endphp
                            <button
                                wire:click="{{ $isOutOfStock ? '' : 'addItemQuick(' . $product['id'] . ')' }}"
            ...
            @else
                <div>
                    @if($popularProducts->isNotEmpty() && $selectedCategoryId === 0 && empty($search))
                        <div class="flex items-center gap-1.5 mb-3">
                            <h3 class="text-xs font-semibold text-stone-500 uppercase tracking-wider">All Items</h3>
                        </div>
                    @endif
                        ...
                        @foreach($regularProducts as $product)
                            ...
                            >
                                <div class="relative mb-2.5">
                                    ...
                                    @if(!empty($product['tags']))
                                        <div class="absolute top-1.5 left-1.5 flex flex-wrap gap-1">
                                            @foreach((array) $product['tags'] as $tag)
                                                @if($tag === 'new')
                                                    <span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-blue-500 text-white shadow-sm">NEW</span>
                                                @elseif($tag === 'signature')
                                                    <span class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-purple-500 text-white shadow-sm">SIGNATURE</span>
                                                @endif
                                            @endforeach
                                        </div>
                                    @endif
                                    @if($isOutOfStock)
                                        <div class="absolute inset-0 bg-black/40 rounded-lg flex items-center justify-center">
                                            <span class="text-white text-xs font-bold uppercase tracking-wider shadow-sm">Sold Out</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="flex items-center gap-1 mt-1.5">
                                    @if($hasOptions)
                                        <span class="text-[10px] font-medium bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded">Options</span>
                                    @endif
                                    @if($isLowStock && !$isOutOfStock)
                                        <span class="text-[10px] font-medium bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded">{{ $product['stock_quantity'] }} left</span>
                                    @endif
                                </div>
                            </button>
                        @endforeach
    ...
    <div class="w-[380px] bg-white border-l border-stone-200 flex flex-col shrink-0">
        ...
        <div class="px-5 py-4 border-b border-stone-100">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <h2 class="font-bold text-stone-800">Cart</h2>
                </div>
                <div class="flex items-center gap-2">
        ...
        <div class="flex-1 overflow-y-auto">
            @if(empty($this->cartItems))
                <div class="flex flex-col items-center justify-center h-full text-stone-400 px-6">
                    ...
                    <p class="font-medium text-stone-500">Cart is empty</p>
                    <p class="text-sm text-stone-400 mt-1">Tap a product to add it</p>
                </div>
            @else
                        ...
                        <div class="px-5 py-3 hover:bg-stone-50 transition-colors">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-stone-800 text-sm leading-tight">{{ $item['product_name'] ?? ($item['product']['name'] ?? 'Unknown') }}</h4>
                                    ...
                                    @endif
                                    <p class="text-xs text-stone-400 mt-1">${{ number_format($item['unit_price'], 2) }} each</p>
                                </div>
        ...
        @if($this->itemCount > 0)
            <div class="border-t border-stone-200 bg-white">
                {{-- Order Notes --}}
                <div class="px-5 pt-3">
                    <input type="text" wire:model.live="orderNotes" placeholder="Order notes..."
                    ...
                    <div class="flex items-center gap-1.5">
                        <button wire:click="applyDiscountPercent(10)"
                            class="text-[10px] font-medium px-2 py-1 rounded border transition-colors
                                {{ $this->order && $this->order->discount > 0 && ($this->order->discount / $this->subtotal * 100) > 9.5 && ($this->order->discount / $this->subtotal * 100) < 10.5 ? 'bg-green-50 border-green-300 text-green-700' : 'border-stone-200 text-stone-500 hover:border-stone-300' }}">
                            10%
                        </button>
                        <button wire:click="applyDiscountPercent(20)"
                            class="text-[10px] font-medium px-2 py-1 rounded border transition-colors
                                {{ $this->order && $this->order->discount > 0 && ($this->order->discount / $this->subtotal * 100) > 19.5 && ($this->order->discount / $this->subtotal * 100) < 20.5 ? 'bg-green-50 border-green-300 text-green-700' : 'border-stone-200 text-stone-500 hover:border-stone-300' }}">
                            20%
                        </button>
                        <button wire:click="applyDiscountPercent(50)"
                            class="text-[10px] font-medium px-2 py-1 rounded border transition-colors
                                {{ $this->order && $this->order->discount > 0 && ($this->order->discount / $this->subtotal * 100) > 49.5 && ($this->order->discount / $this->subtotal * 100) < 50.5 ? 'bg-green-50 border-green-300 text-green-700' : 'border-stone-200 text-stone-500 hover:border-stone-300' }}">
                            50%
                        </button>
                ...
                <div class="px-5 py-3 space-y-1.5">
                    ...
                    <div class="flex justify-between text-sm">
                        <span class="text-stone-500">Tax ({{ config('pos.tax_rate', 0.10) * 100 }}%)</span>
                ...
                <div class="p-6">
                    ...
                    <div class="flex items-center justify-between mb-5">
                        <div>
                            ...
                            <p class="text-sm text-stone-400">Customize your order</p>
                        </div>
                    ...
                    @if(!empty($selectedProduct['variants']))
                        ...
                        @if($activeVariants->isNotEmpty())
                            <div class="mb-5">
                                <label class="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Size</label>
                                <div class="grid grid-cols-3 gap-2">
                                    @foreach($activeVariants as $variant)
                                        <button
                                            wire:click="$set('selectedVariant', {{ json_encode($variant) }})"
                            ...
                            <div class="mb-5">
                                <div class="flex items-center justify-between mb-2">
                                    ...
                                    @if($group['max_selections'] > 1)
                                        <span class="text-[10px] text-stone-400 font-medium">Max {{ $group['max_selections'] }}</span>
                                    @endif
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    @foreach($group['options'] as $option)
                                        @if($option['is_active'])
                                            ...
                                            @endphp
                                            <button
                                                wire:click="toggleModifier({{ $group['id'] }}, {{ $option['id'] }}, '{{ $group['name'] }}', '{{ $option['name'] }}', {{ $option['price'] }}, {{ $group['max_selections'] }}, {{ $group['is_required'] ? 'true' : 'false' }})"
                    ...
                    {{-- Quantity --}}
                    <div class="mb-5">
                        <label class="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Quantity</label>
                    ...
                    {{-- Notes --}}
                    <div class="mb-5">
                        <label class="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Special Instructions</label>
                        <input type="text" wire:model.live="itemNotes" placeholder="e.g. Extra hot, no sugar..."
                    ...
                    <div class="bg-stone-50 rounded-xl p-4 mb-5 space-y-1.5">
                        <div class="flex justify-between text-sm">
                            <span class="text-stone-500">Base price</span>
                        ...
                        @if(collect($selectedModifiers)->sum('price') > 0)
                            <div class="flex justify-between text-sm">
                                <span class="text-stone-500">Modifiers</span>
                        ...
                        <div class="flex justify-between pt-2 border-t border-stone-200">
                            <span class="font-bold text-stone-800">Unit Price</span>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button wire:click="confirmAddItem"
                            ...
                            Add &minus; ${{ number_format($unitPrice * $itemQuantity, 2) }}
                        </button>
                    </div>
                </div>
            </div>
    ...
    @if($showPaymentModal)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md"
                ...
                x-transition:enter-end="opacity-100 scale-100">
                <div class="p-6">
                    <h3 class="text-lg font-bold text-stone-800 mb-5">Process Payment</h3>

                    {{-- Payment Method --}}
                    <div class="mb-5">
                        <label class="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Method</label>
                        <div class="grid grid-cols-3 gap-2">
                            ...
                            <button wire:click="selectPaymentMethod('card')"
                                class="py-3 px-4 rounded-xl border-2 text-sm font-semibold transition-all flex flex-col items-center gap-1
                                    {{ $paymentMethod === 'card' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-stone-200 hover:border-stone-300 text-stone-600' }}">
                    ...
                    @if($paymentMethod === 'cash')
                        {{-- Quick Cash Buttons --}}
                        <div class="mb-4">
                            <label class="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Quick Amount</label>
                            <div class="grid grid-cols-4 gap-2">
                                @php
                                    $denominations = [1, 5, 10, 20, 50, 100];
                                    $quickAmounts = array_filter($denominations, fn($d) => $d >= $this->total);
                        ...
                        {{-- Amount Tendered --}}
                        <div class="mb-5">
                            <label class="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Amount Tendered</label>
                    ...
                    <div class="bg-stone-50 rounded-xl p-4 mb-5 space-y-2">
                        <div class="flex justify-between text-lg font-bold">
                            <span class="text-stone-800">Total Due</span>
                    ...
                    <div class="flex gap-3">
                        ...
                        @if($paymentMethod === 'cash')
                            <button wire:click="processPayment"
                                class="flex-1 px-4 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold transition-colors shadow-sm shadow-green-200
    ...
    @if($showKhqrModal && $khqrData)
        <div class="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md"
                ...
                <div class="p-6">
                    <div class="flex items-center justify-between mb-5">
                        <h3 class="text-lg font-bold text-stone-800">KHQR Payment</h3>
                        <button wire:click="cancelKhqr" class="w-8 h-8 rounded-full bg-stone-100 hover:bg-stone-200 flex items-center justify-center transition-colors">
                            <svg class="w-4 h-4 text-stone-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        </button>
                    ...
                    <div class="text-center mb-5">
                        <p class="text-sm text-stone-500 mb-3">Scan with your banking app</p>
                        <div class="bg-white border-2 border-stone-100 rounded-xl p-4 inline-block">
                            <img src="{{ $khqrData['qr_image_url'] }}" alt="KHQR Code" class="w-48 h-48">
                        </div>
                    ...
                    <div class="bg-stone-50 rounded-xl p-4 mb-5 text-center">
                        <p class="text-xs text-stone-400 mb-1">Expires in</p>
                    ...
                    <div class="flex gap-3">
                        <button wire:click="checkKhqrStatus"
                            class="flex-1 px-4 py-3 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-bold transition-colors">
                            Check Payment
                        </button>
                    ...
                    <p class="text-xs text-stone-400 text-center mt-4">
                        QR code auto-verifies when payment is received
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md"
                ...
                <div class="p-6">
                    <div class="text-center mb-5">
                        ...
                        <h3 class="text-lg font-bold text-stone-800">Payment Complete</h3>
                        <p class="text-sm text-stone-400 mt-1">Order has been processed successfully</p>
                    </div>
